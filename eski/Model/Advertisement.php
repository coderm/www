...
class Advertisement extends AppModel {
    ...
    public function homePageAdverts($count = 10) {
    ...
    public function lastBooksAdverts($count = 10) {
    ...
    public function getAdvertDetails($result) {
        ...
        $result['Advertisement']['details'] = unserialize($result['Advertisement']['details']);
        $result['Advertisement']['picture'] = unserialize($result['Advertisement']['picture']);
        $result['Advertisement']['conditions'] = unserialize($result['Advertisement']['conditions']);
        /* $result['Advertisement']['phoneNumber'] = unserialize($result['Advertisement']['phoneNumber']);
          $result['Advertisement']['phoneCountryCode'] = $result['Advertisement']['phoneNumber']['phoneCountryCode'];
          $result['Advertisement']['phoneNumber'] = $result['Advertisement']['phoneNumber']['phoneNumber']; */
        $result['Advertisement']['type'] = $result['Advertisement']['advert_class_id'];
        $result['Advertisement']['maxGuests'] = $result['Advertisement']['max_guests'];
        $result['Advertisement']['currency'] = $result['Advertisement']['currency_id'];
        $result['Advertisement']['country'] = $result['Advertisement']['country_id'];
        $result['Advertisement']['city'] = $result['Advertisement']['city_id'];
        $result['Advertisement']['district'] = $result['Advertisement']['county_id'];
        $result['Advertisement']['subDistrict'] = $result['Advertisement']['neighborhood_id'];
        $result['Advertisement']['subDistrictText'] = $result['Advertisement']['subDistrict_text'];
        ...
        $result['Search']['advert_class'] = $this->AdvertClass->idToMessageTextId($result['Advertisement']['advert_class_id']);
        $result['Search']['currency'] = $this->CurrencyUnit->idToMessageTextId($result['Advertisement']['currency_id']);
        $result['Search']['country'] = $this->Country->idToMessageTextId($result['Advertisement']['country_id']);
        $result['Search']['city'] = $this->City->idToMessageTextId($result['Advertisement']['city_id']);
        $result['Search']['district'] = $this->County->idToMessageTextId($result['Advertisement']['county_id']);
        $result['Search']['subDistrict'] = $this->Neighborhood->idToMessageTextId($result['Advertisement']['neighborhood_id']);
        return $result;
    }
    ...
    public function getAdvert($advertId) {
        /*
          $results = Cache::read('vw_get_advert_details_' . $advertId, 'monthly');
          if (!$results) {
          $results = $this->query('SELECT * FROM vw_get_advert_details WHERE advert_id=' . $advertId);
          Cache::write('vw_get_advert_details_' . $advertId, $results, 'monthly');
          }
          ...
          $advert = array();
          foreach ($results as $result) {
          $key = $result['vw_get_advert_details']['detail_class'];
          $type = $result['vw_get_advert_details']['detail_class_type'];
          $elementType = $result['vw_get_advert_details']['form_element_type'];

          if (isset($advert[$type])) {
          if (!is_array($advert[$type])) {
          $advert[$type] = array();
          $advert[$type][] = $result['vw_get_advert_details'];
          }
          $advert[$type][] = $result['vw_get_advert_details'];
          }
          else
          ...
          $props = $this->parseProperties($picture['detail']);
          $picture['imageName'] = $props['name'];
          $picture['imageOrder'] = $props['order'];
          $a[] = $picture;
          $orders[] = $props['order'];
          }
          array_multisort($orders, $a);
          $advert['lcdt_picture'] = $a;


          $detailsToParsePropKeys = array();
          $detailsToParsePropKeys[] = 'lcdt_map';
          $detailsToParsePropKeys[] = 'lcdt_city';
          $detailsToParsePropKeys[] = 'lcdt_county';
          $detailsToParsePropKeys[] = 'lcdt_neighborhood';
          $detailsToParsePropKeys[] = 'lcdt_district';



          foreach ($detailsToParsePropKeys as $detailKey) {
          if (isset($advert[$detailKey])) {
          $detail = $advert[$detailKey][0]['detail'];
          $props = $this->parseProperties($detail);
          $advert[$detailKey][0]['detailPrperties'] = $props;
          }
          }


          $result = $this->query('SELECT COUNT(*) AS isPartner FROM dt_advert_details dad JOIN tx_users_groups tug ON dad.advert_detail = tug.user_group_sellect_id WHERE tug.group_id = 12 AND dad.detail_class_id = 66 AND dad.advert_id =' . $advertId);

          $advert['isPartner'] = array($result[0][0]['isPartner'] > 0); */
        return NULL;
    }

    public function getMyHouses($userId, $userPass) {
        ...
        if ($user['0']['lsm']['type'] == 'success') {
            return $this->query('SELECT
                         ...
                         ,da.advert_class_id
                         ,da.status
                         ,title.advert_detail title
                        FROM
                          dt_adverts da
                          JOIN dt_advert_details hh
                            on da.advert_id = hh.advert_id and
                               hh.detail_class_id = 66
                          JOIN dt_advert_details title
                            on da.advert_id = title.advert_id and
                               title.detail_class_id = 4
                        WHERE
                          da.status <> "deleted" and
                          hh.advert_detail =' . $userId);
        } else {
            return $user['0']['lsm']['message_text_id'];
        }
    }

    public function getAdvert2($advertId) {
        //  $results = $this->query('SELECT * FROM vw_get_adverts WHERE advert_id=' . $advertId);
        //  $result = $results[0];
        //$result['vw_get_adverts']['picture'] = $this->parseProperties($result['vw_get_adverts']['picture']);
        return $result;
    }
    ...
    public function getSimilarAdverts($advertId, $baseAdvertId, $limit = 5) {
        $results = Cache::read('similar_adverts_' . $advertId, 'monthly');
        if (!$results) {
            $sql = 'SELECT * FROM (SELECT COUNT(vsa.advert_detail_id) similarity ,vsa.* FROM vw_similar_adverts vsa WHERE vsa.main_advert_id = ' . $advertId . ' AND vsa.advert_base_id <> ' . $baseAdvertId . ' GROUP BY vsa.advert_id) similar group by similar.advert_base_id ORDER BY similar.similarity DESC LIMIT ' . $limit;
            $results = $this->query($sql);
            Cache::write('similar_adverts_' . $advertId, $results, 'monthly');
        }
        return $results;
    }

    public function getBaseAdverts($advertId, $limit) {
        $results = Cache::read('base_adverts_' . $advertId, 'monthly');
        if (!$results) {
            $sql = 'SELECT
                      *
                    FROM
                      (SELECT
                         COUNT(vsa.advert_detail_id) similarity
                        ,vsa.*
                       FROM
                         vw_similar_adverts vsa
                         JOIN dt_advert_details mdad
                           ON vsa.main_advert_id = mdad.advert_id and
                              mdad.detail_class_id = 88
                         JOIN dt_advert_details dad
                           ON vsa.advert_id = dad.advert_id and
                              dad.detail_class_id = 88 and
                              mdad.advert_detail = dad.advert_detail
                       WHERE
                         vsa.main_advert_id = ' . $advertId . '
                       GROUP BY
                         vsa.advert_id) similar
                    group by
                      similar.advert_base_id
                    ORDER BY
                      similar.similarity DESC
                    limit ' . $limit;
            $results = $this->query($sql);
            Cache::write('base_adverts_' . $advertId, $results, 'monthly');
        }
        return $results;
    }

    public function getHouseholdersSimilarAdverts($advertId, $limit) {
        $results = Cache::read('householder_similar_adverts_' . $advertId, 'monthly');
        if (!$results) {
            $sql = 'SELECT
                      *
                    FROM
                      (SELECT
                       ...
                       FROM
                         ...
                         JOIN dt_advert_details mdad
                           ON vsa.main_advert_id = mdad.advert_id and
                              mdad.detail_class_id = 66
                         JOIN dt_advert_details dad
                           ON vsa.advert_id = dad.advert_id and
                              dad.detail_class_id = 66 and
                              mdad.advert_detail = dad.advert_detail
                       WHERE
                         vsa.main_advert_id = ' . $advertId . '
                       GROUP BY
                    ...
                    limit ' . $limit;
            $results = $this->query($sql);
            Cache::write('householder_similar_adverts_' . $advertId, $results, 'monthly');
        }
        return $results;
    }

    public function getLastBookingAdverts($limit = 5) {
        $results = Cache::read('Last_Bookings_adverts_' . $limit, 'halfHourly');
        if (!$results) {
            $results = $this->query('SELECT * FROM vw_last_booking_adverts LIMIT ' . $limit);
            Cache::write('Last_Bookings_adverts_' . $limit, $results, 'halfHourly');
        }
        return $results;
    }
    ...
    public function proposedAdvert($limit = 5) {
        // $results = $this->query('select * from vw_get_adverts where advert_status = "active" order by on_top_date desc , show_in_homepage desc, is_partner desc LIMIT ' . $limit);
        return $results;
    }
    ...
    public function getBaseAdvertIdByAdvert($advert) {
        if (isset($advert['lcdt_base_advert'][0]['detail']))
            $result = $advert['lcdt_base_advert'][0]['detail'];
        else
            $result = $advert['lcdt_title'][0]['advert_id'];
